...
async function initCasbin() {
  ...
  // Check if `TypeORMAdapter.newAdapter` exists; if not, use `TypeORMAdapter.default.newAdapter`
  if (TypeORMAdapter.newAdapter) {
    adapter = await TypeORMAdapter.newAdapter(config);
  } else {
    adapter = await TypeORMAdapter.default.newAdapter(config);
  }
...
export { addPolicyInDomain, addRoleForUserInDomain, casbinMiddleware, closeCasbinAdapter, getPoliciesInDomain, getRolesInDomain, getUserType, getUserTypeInDomain, getUsersForRoleInDomain, listRolesForUser, listRolesForUserInDomain, listRolesForUserInDomains, removePoliciesInDomain, removeRoleForUserInDomain, removeRolesForUserInAllDomains, removeRolesForUserInDomain, setupCasbinMiddleware };
